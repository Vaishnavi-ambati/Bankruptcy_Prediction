...
class modelFinder:
    ...
    def __init__(self, loggerObj, log_file):
        ...
        self.preprocess = dataPreprocessor.processData(self.loggerObj, self.log_file)
    ...
    def get_best_params_for_xgboost(self,df_train_complete):
        ...
        try:
            ...
            df_train_down_up_sampled = self.preprocess.up_and_down_sampling(df_train_complete, 20, 'class')
            X_train_sampled = df_train_down_up_sampled.drop('class', axis=1)
            y_train_sampled = df_train_down_up_sampled['class']
            ...
            self.log_file = self.loggerObj.write_log(self.log_file, 'Best hyperparmeters have been found for 1year and the model has been trained with best hyperparamters.')
    ...
    def get_best_model(self,df_train_complete,df_test_complete, year):
        ...
        # create best model for 1year
        try:
            self.log_file = self.loggerObj.write_log(self.log_file,'Initiated finding the best parameters for 1year model.')

            print("Inside get best model")

            self.xgboost= self.get_best_params_for_xgboost(df_train_complete)

            X_test = df_test_complete.drop('class', axis=1)
            y_test = df_test_complete['class']

            self.prediction_xgboost = self.xgboost.predict(X_test) # Predictions using the 1year Model

            if len(y_test.unique()) == 1:
                self.xgboost_score = accuracy_score(y_test, self.prediction_xgboost)

            else:
                self.xgboost_score = f1_score(y_test, self.prediction_xgboost, average='micro') # AUC for 1year
            self.log_file = self.loggerObj.write_log(self.log_file, 'Best hyperparmeters have been found for 1year')
            ...
            if(self.random_forest_score <  self.xgboost_score):
                ...
                self.log_file = self.loggerObj.write_log(self.log_file, 'Best Model after comparing the scores is 1year.')
                self.log_file = self.loggerObj.write_log(self.log_file, 'Exiting get_best_model method.')

                return self.xgboost, str(year) + ' year','XGBoost'
            else:
